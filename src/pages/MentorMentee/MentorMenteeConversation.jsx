...
const MentorMenteeConversation = () => {
 ...
 useEffect(() => {
   if (!selectedStudent || !semester || !user) return;


   api.get("/conversations/check", {
     params: {
       mentorId: user._id,
   ...
   .then((res) => {
     if (res.data.count >= 2) {
       enqueueSnackbar(
         "2 offline conversations already completed for this semester",
         { variant: "info" }
       );
     }
   })
